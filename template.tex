...
Models are often used to predict heat transfer coefficients, which are strongly linked to the aerodynamic performance of the component. A small error in prediction of aerodynamic performance leads to a large error in the prediction of heat transfer coefficient~\cite{Nikparto2018}. The accuracy of heat transfer models is dependant on the turbulence model, the approach to three-dimensional modelling, and the density of geometric grid. An example of comparison between experimental measurements and the modelling of turbine blade heat transfer shows that there can be discrepancies of up to 40\si{\degreeCelsius} on the surface of a blade when the upstream gas temperature is 614\si{\degreeCelsius}~\cite{Shevchenko2018}. Guan \textit{et al.}~\cite{Guan2019} compared the results of a coupled model to experimental measurements from temperature sensitive paints, which provided comparable results when film cooling was taken into account. A comprehensive review of conjugate heat transfer models has been carried out by Samarasinghe~\cite{Samarasinghe2019}.

\subsection{Review of other studies done in COMSOL for harsh environment sensing}
\textcolor{blue}{Items to add: Piezoelectric models, temperature variation models, guided wave models. If they compare their work with experiments, then talk about that too if not explain that in this work you at least validated part of your model with experiments.}
...
\textcolor{red}{not sure on the need for this section.}

\section{Lamb wave mode targeting}

The COMSOL model described in the next section is based on the experimental setup described in Section~\ref{experiments}. The experimental setup has been used to analyse the effect of temperature on wave propagation in an Aluminium plate. Dispersion curves have been generated based on material properties to determine a suitable excitation frequency, and to extract the temperature sensitivity of the selected mode. The $S_0$ Lamb wave mode has been targeted at a frequency-thickness product of 1 MHz-mm (in a 1 mm plate). The velocities extracted from dispersion curves can be seen plotted against experimental and simulated results in Figure~\ref{fig:s0result}.
...
The multiphysics simulation package COMSOL has been used to simulate a potential temperature monitoring system, as described experimentally in Section~\ref{experiments}.
